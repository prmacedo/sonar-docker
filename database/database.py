...
class Database:
    ...
    def save(self):
        with self.engine.connect() as conn:
            ...
            if len(self.data) == 0:
                return False
            for data, table_name in self.data:
                ...
                if isinstance(data, list):
                    ...
                    group[table_name].extend(list_dict)
                else:    
                    group[table_name].append(data)
            
            for table_name, values in group.items():
                table = Table(table_name, self.metadata, autoload_with=self.engine)
                self.session.execute(table.insert().prefix_with("OR REPLACE"), values)
            self.session.commit()
            self.data = []
            self.table_name = None
        return True
                ...
                # if clean_local:
                #     # self.backup_database(clean_local=clean_local)
                #     pass
